...
int rop_zset_set_string(redisContext *conn, char *key, char *value)
{
	int score = 0;
 ...
 */
/* -------------------------------------------*/
int rop_insert_hash(redisContext *conn, char *hashkey, char *field,char* value){
	int score = 0;

	redisReply *reply = NULL;

	reply = redisCommand(conn, "HMSET %s %s %s", hashkey, field,value);
  
  rop_test_reply_type(reply);
    
	if (reply->type != REDIS_REPLY_STRING) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]ZSCORE %s %s %s error %s\n", hashkey, field,field,conn->errstr);
        score = -1;
		goto END;
	}
...
/* -------------------------------------------*/
/**
 * @brief  给指定的hash表 获得指定的field对应的value值
 *
 * @param conn            已建立好的链接
 * @param key           	 hash表名
 * @param field            hash表下的区域名    
 ...
 */
/* -------------------------------------------*/
int rop_GetHash_one_field(redisContext *conn, char *key, char *field, char* outbuf){
	int score = 0;
  ...
  rop_test_reply_type(reply);
	...
	if (reply->type != REDIS_REPLY_STRING) {
		LOG(REDIS_LOG_MODULE, REDIS_LOG_PROC, "[-][GMS_REDIS]ZSCORE %s %s %s error %s\n", key, field,outbuf,conn->errstr);
        score = -1;
		goto END;
	}
	
	strncpy(outbuf,reply->str,reply->len);
	LOG("2222", "3333", " %s || %s\n", outbuf, reply->str);
  score = atoi(reply->str);
